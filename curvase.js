var Curvase = (function() {
    ...
    BezierEditor.prototype._draw = function() {
        ...
        if (!this.isSpeedMode) {
            ...
            for (var hi = 0; hi < this.points.length; hi++) {
                ...
                if (hp.hOutX !== null) this._drawHandle(ctx, hp.hOutX, hp.hOutY, "#ff8c42");
                if (hp.hInX !== null) this._drawHandle(ctx, hp.hInX, hp.hInY, "#4a9eff");
            }
        ...
        var PADDING = 40; 
        ctx.fillStyle = hasBg ? "rgba(255,255,255,0.55)" : "rgba(255,255,255,0.25)";
        ...
        ctx.textAlign = "right";
        ctx.fillText(yMinL, PADDING - 6, PADDING + gs + 3);
        ctx.fillText(yMaxL, PADDING - 6, PADDING + 3);
    };

    BezierEditor.prototype._drawHandle = function(ctx, x, y, color) {
        var cx = this._toX(x);
        var cy = this._toY(y);
        ctx.beginPath();
        ctx.fillStyle = color;
    ...
    BezierEditor.prototype.getSegments = function() {
        ...
        for (var i = 0; i < this.points.length - 1; i++) {
            ...
            var dx = b.x - a.x;
            var dy = b.y - a.y;
            var sx1 = dx > 0.0001 ? (a.hOutX - a.x) / dx : 0;
            var sy1 = Math.abs(dy) > 0.0001 ? (a.hOutY - a.y) / dy : sx1;
            var sx2 = dx > 0.0001 ? (b.hInX - a.x) / dx : 1;
            var sy2 = Math.abs(dy) > 0.0001 ? (b.hInY - a.y) / dy : sx2;
